...
- [一、问题回顾](#一问题回顾)
  - [1.8 端到端下发延迟分析](#18-端到端下发延迟分析)
  - [1.9 覆盖索引 vs 回表：IO 模型深入分析](#19-覆盖索引-vs-回表io-模型深入分析)
- [二、优化方案概览](#二优化方案概览)
...
DB 查询优化的收益需要放在整条链路中评估，才能判断是否真的影响任务下发延迟。

#### 任务下发全链路

```
Action INSERT 入库 (state=0)
       ↓
RedisActionLoader 轮询 (每 100ms)
       ↓  loadActions() → 查 DB → 写 Redis
Agent gRPC Fetch (每 3-5s)
       ↓  从 Redis ZPOPMIN 拉取
Agent 开始执行
```

**任务下发延迟 = 等待轮询 + DB 查询 + Redis 写入 + Agent 拉取等待**

#### 各场景耗时估算（A2 优化后）

| 场景 | Action 数量 | DB 查询 | Redis Pipeline | DB 更新状态 | 单轮总计 |
|------|------------|---------|---------------|------------|---------|
| 常规 Stage | 200 行 | < 2ms | < 1ms | ~2ms | **~5ms** |
| 中等 Stage | 2000 行 | ~3-5ms | ~5ms | ~10ms | **~20ms** |
| 大集群全量操作 | 6000 行 | ~10ms | ~15ms | ~30ms | **~55ms** |
| 5 Job 并行 × 2000 行 | 10000 行 | 5 × 5ms | 5 × 5ms | 5 × 10ms | **~100ms** |

以上所有场景，单轮处理时间都 ≤ 100ms 的轮询间隔，**不会积压**。

#### 真正的延迟瓶颈

```
DB 查询 + Redis 写入：5-55ms     ← 不是瓶颈
RedisActionLoader 轮询间隔：100ms（最坏等 100ms）  ← 不是瓶颈
Agent gRPC 轮询间隔：3-5s（最坏等 5s）            ← 真正的瓶颈
```

**结论**：任务下发延迟 ≈ **2-3s**，主要由 Agent 的 gRPC 轮询周期决定。DB 查询层面不管是 200 行还是 6000 行，都不是延迟来源。要进一步优化下发延迟，方向是把 Agent 的拉模式改成推模式（如 Redis Pub/Sub 通知），但那是另一个层面的架构改造。

### 1.9 覆盖索引 vs 回表：IO 模型深入分析

Step 8 覆盖索引将查询从 200ms 降到 20ms，为什么消除回表能有 10 倍提升？需要理解两者的 IO 模型差异。
...
覆盖索引（Using index）：
  B+ 树叶子节点 → 顺序读 → 直接返回
  ┌─────┬─────┬─────┬─────┐
  │ row1│ row2│ row3│ row4│  ← 叶子节点物理上连续，顺序扫描
  └─────┴─────┴─────┴─────┘
  IO 类型：顺序读（Sequential I/O）

回表查询（Using index condition）：
  二级索引 → 拿到主键 id → 回主键索引查完整行 → 每行一次随机 IO
  idx_state     主键索引（聚簇索引）
  ┌────┐       ┌─────────────────────┐
  │id=5│ ───→  │ 跳到 id=5 的页      │  ← 随机 IO #1
  │id=88│───→  │ 跳到 id=88 的页     │  ← 随机 IO #2
  │id=302│──→  │ 跳到 id=302 的页    │  ← 随机 IO #3
  └────┘       └─────────────────────┘
  IO 类型：随机读（Random I/O）
```

#### 为什么回表是随机 IO

```
二级索引的叶子节点按 (state, id) 排序：
  state=0, id=5
  state=0, id=88
  state=0, id=302
  state=0, id=1547
...
这些 id 在主键聚簇索引中的物理位置是分散的：
  页 #12:  id=1~20      ← id=5 在这里
  页 #57:  id=81~100     ← id=88 在这里
  页 #198: id=301~320    ← id=302 在这里
  页 #823: id=1540~1560  ← id=1547 在这里

每次回表都要跳到不同的页 → 随机 IO
```

#### 耗时对比：热数据（Buffer Pool 命中）

| 行数 | 覆盖索引 | 回表查询 | 差距 |
|------|---------|---------|------|
| 200 行 | < 1ms | ~2-3ms | 2-3x |
| 2000 行 | ~2ms | ~15-20ms | **10x** |
| 6000 行 | ~5ms | ~50-60ms | **10x** |

热数据下差距有限，因为随机读命中的是内存页，每次回表 ~5-10μs。

#### 耗时对比：冷数据（需要磁盘 IO）

| 行数 | 覆盖索引 | 回表（HDD） | 回表（SSD） |
|------|---------|------------|------------|
| 200 行 | < 1ms | **200 × 10ms = 2s** | 200 × 0.1ms = 20ms |
| 2000 行 | ~2ms | **2000 × 10ms = 20s** | 2000 × 0.1ms = 200ms |
| 6000 行 | ~5ms | **6000 × 10ms = 60s** | 6000 × 0.1ms = 600ms |

- **HDD 随机 IO**：~10ms/次（寻道时间）
- **SSD 随机 IO**：~0.1ms/次
- **覆盖索引顺序读**：不管 HDD 还是 SSD 都很快，因为是连续页

#### 为什么 action 表特别受回表影响

action 表包含 `command_json`、`stdout`、`stderr` 这些 **longtext 字段**，这意味着：

1. **单行数据大** → 一个 16KB 的页放不了几行 → 数据页多且分散
2. **Buffer Pool 装不下所有数据页** → 冷数据回表大概率要磁盘 IO
3. **覆盖索引只需要 `id` + `hostuuid`** → 几十字节/行，索引叶子节点紧凑，大概率全部在内存

所以在 action 表场景下：

```
覆盖索引查 2000 行：~2ms（索引页全在内存）
回表查 2000 行：    ~20ms（热）到 ~200ms+（冷，SSD）
...
| 优化 | 解决的问题 | 手段 | 效果 |
|------|-----------|------|------|
| Step 8 覆盖索引 | 消除回表随机 IO | 把查询需要的列放进索引 | 200ms → 20ms |
| A2 Stage 维度查询 | 缩小索引扫描范围 | 用高选择性前缀列缩小扫描区间 | 20ms → 2ms |

**面试表达**："先消除回表（200ms→20ms），再缩小范围（20ms→2ms），两步优化各解决一个独立的性能瓶颈。"
...
### 3.5.1 深入分析：为什么都是覆盖索引，扫描行数却差 250 倍？

优化前后都是覆盖索引（`Using index`，不回表），但查询耗时从 20ms 降到 < 2ms。关键不在于"要不要回表"，而在于**索引前缀列的选择性（Selectivity）不同**，导致索引内部扫描的范围差异巨大。

**覆盖索引解决的问题**：不用回表（不需要再去主键索引拿完整行）。
**前缀列选择性决定的问题**：在索引 B+ 树内部要扫描多少行。

#### 优化前：低选择性前缀

```
索引: idx_action_state_host_cover (state, id, hostuuid)
SQL:  WHERE state = 0 LIMIT 2000
```

- `state = 0` 是**低选择性**条件——百万级 action 表中 `state = 0` 的行可能有 **50000** 行
- 索引定位到 `state = 0` 区间后，要在这 50000 行中顺序扫描，然后 LIMIT 2000 截断
- MySQL 优化器的 estimated rows 就是 ~50000

#### 优化后：高选择性前缀

```
索引: idx_action_stage_state_cover (stage_id, state, id, hostuuid)
SQL:  WHERE stage_id = ? AND state = 0
```

- `stage_id = ?` 是**高选择性**条件——运行中的 Stage 通常只有 1 个，名下 action 通常只有 **200** 行
- 索引先通过 `stage_id` 定位到极小的区间（200 行），再在其中过滤 `state = 0`
- 扫描范围从 50000 → 200

#### 用图理解扫描范围的区别

```
优化前索引 (state, id, hostuuid)：
┌──────────────────────────────────────────────────────┐
│  state=0 的所有 action（~50000 行）                     │
│  ┌──────┬──────┬──────┬──────┬─── ... ──┬──────┐     │
│  │ row1 │ row2 │ row3 │ row4 │          │row50k│     │
│  └──────┴──────┴──────┴──────┴─── ... ──┴──────┘     │
│  ←────────── 在这 50000 行中扫描 ──────────→ LIMIT 2000│
└──────────────────────────────────────────────────────┘

优化后索引 (stage_id, state, id, hostuuid)：
┌──────────────────────────────┐
│  stage_id=123, state=0       │
│  ┌──────┬──────┬─────┬─────┐ │
│  │ row1 │ row2 │ ... │r200 │ │
│  └──────┴──────┴─────┴─────┘ │
│  ←── 只扫这 200 行 ──→       │
...
**覆盖索引决定「要不要回表」，前缀列的选择性决定「要扫多少行」。** 优化前后都不回表，但 `stage_id` 作为索引前缀列把扫描范围从"全局 state=0 的 5 万行"缩小到"单个 Stage 下 state=0 的 200 行"——这才是 10 倍性能差异的来源。
